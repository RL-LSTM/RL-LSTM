...
class DDPGAgent:
	def __init__(self, actor, critic, replay_buffer, batch_size=64, gamma = 0.99, 
				seed=1234, episode_len=1000, episode_steps=1000,noise_mean = 0.0, 
				noise_th=0.15, noise_std=0.20, noise_decay=25):
	...
	def load_actor_weights(self, save_dir='../saver', filename='sample_critic'):
		PATH = save_dir + '/' + filename
	...
	def train(self,env):
		...
		for i in tqdm(range(self.episode_len)):
			...
			s = env.reset()
			exploration_noise = OUNoise(actor.action_dim, self.noise_mean, self.noise_th, self.noise_std, self.seed + i)

			for j in range(self.episode_steps):
				env.render()
				input_state  = np.reshape(s, (1, actor.state_dim))
				...
				self.replay_buffer.add(np.reshape(s, (actor.state_dim,)),
								  np.reshape(a, (actor.action_dim,)),
								  r, terminal, np.reshape(s2, (actor.state_dim,)))
				...
				if self.replay_buffer.size() > self.batch_size:
					...
					critic_output = target_critic.predict_target(s2_batch, target_actor.predict(s2_batch))
					critic_output = critic_output.data.numpy()
					...
					if CUDA: 
						...
						y = y.cuda()
					critic.train(s_batch, a_batch, y)
					actor.train(s_batch, critic)
					target_actor.update_target_network(actor)
					target_critic.update_target_network(critic)
				else:
					loss = 0
