...
class A2C(object):
	def __init__(self, actor_critic, rollout, learning_rate, 
		...
		self.optimizer = optim.RMSprop(self.actor_critic.parameters(), self.learning_rate, eps=self.eps, alpha=self.alpha)
		...
		for j in range(self.num_updates):
			...
			if self.vis and j % self.vis_interval == 0:
				try:
					...
					win = visdom_plot(viz, win, self.log_dir, self.env_name, 'a2c')
				except IOError:
					pass
